...
def ELMo_module(tokenized_sentences, embedding_size): 
    """
    Slightly different method than the one contained in the ELMo class in the source code folder.
    Here we divide the entire dataset into group of 100 sentences, and then we create embeddings all together rather than one sentence at the time
    ...
    list_sentences =  np.asarray([' '.join(x) for x in tokenized_sentences])
    #Divide in batches of 100 sentences
    batches = [list_sentences[i:i+100] for i in range(0,list_sentences.shape[0],100)]
    sentence_embedding = [elmo_vector(x, elmo) for x in batches]
    #Flatten the returned array
    flattened_embeddings = [item for sublist in sentence_embedding for item in sublist]


    return sequence_length, flattened_embeddings
...
def predictions_architecture_1(test_x, sequence_length, complete_model, graph_path):
    """
    ...
    with tf.Session() as sess:
            ...
            for i in range(len(test_x)): 
                    ...
                    sample = np.reshape(test_x[i], (1, test_x[i].shape[0], test_x[i].shape[1] ))
...
def predictions_architecture_2(test_x, sequence_length, complete_model, graph_path):
    """
    Restore the trained model and feed it with the test set to generate predictions
    ...
    with tf.Session() as sess:
            ...
            inputs = graph.get_tensor_by_name('Inputs:0')
            input_prob = graph.get_tensor_by_name("Probabilities:0")
            seq_length = graph.get_tensor_by_name("Sequence_Length:0")
            #Get the softmax layer
            prediction_tensor = graph.get_tensor_by_name("dense_sensekeys/Sensekey_Logits:0")
            #Feed the model with one sample at the time
            for i in range(len(test_x)): 
                    if i % 100 == 0:
                        print(i)
                    ...
                    padded_x[:sample.shape[0], :sequence_length[i], :sample.shape[2]] = sample[:,:sequence_length[i],:]
                    #Feed the model
                    feed_dict = {input_prob:1.0, inputs:padded_x, seq_length:[sequence_length[i]]}
                    predictions.append(sess.run(prediction_tensor[0], feed_dict))
    predictions = np.asarray(predictions)
    print("Predictions obtained: ", predictions.shape)
    print()
